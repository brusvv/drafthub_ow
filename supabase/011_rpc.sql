-- ════════════════════════════════════════════════════════════
-- DraftHub OW — 011_rpc.sql
-- Применять ПОСЛЕДНИМ: после 001→002→003→007→008→009→010.
--
-- ⚠️ Был 004_rpc.sql, переименован из-за жёсткой зависимости на tier_lists
-- (create_tier_set/set_default_tier_set объявляют `v_set tier_lists%ROWTYPE`).
-- В отличие от обычных SQL-запросов внутри тела функции — они резолвятся
-- лениво, только при первом вызове — %ROWTYPE резолвится СРАЗУ при
-- `CREATE FUNCTION`, во время компиляции DECLARE-блока. Раньше файл шёл
-- под номером 004 (до 007_catalog_tables.sql, который создаёт tier_lists),
-- поэтому CREATE FUNCTION падал с `relation "tier_lists" does not exist`.
-- Больше так не будет — числовой порядок 001..011 теперь совпадает с
-- порядком реальных зависимостей, применять строго по возрастанию номера.
--
