...
-- 004_rpc.sql → 006_hero_counters.sql, ДО 008_catalog_rls.sql и
-- 009_catalog_seed.sql (обе зависят от таблиц отсюда). Бэкфилл не
-- нужен — живой БД с реальными данными ещё нет, можно DROP и
-- пересоздавать напрямую.
--
...
-- контрпиков (в текущей схеме единственный источник team-уровня —
-- heroes.counters jsonb; hero_counters 006 хранит только global/personal).
-- Чтобы не потерять функциональность и следуя духу MIGR-0 ("3 механизма
-- путают"), hero_counters здесь расширен на 3-й scope='team' — теперь
-- ОДНА таблица на все три уровня, тем же паттерном что tier_lists.
-- Если это неверная трактовка — see CHANGELOG, откатить на "оставить
-- counters в heroes" отдельным ALTER.
--
-- ⚠️ MIGR-4 (RPC) и MIGR-2 (data layer) будут ссылаться на объекты,
-- создаваемые здесь — до их завершения create_tier_set/view_shared_tier/
-- db-load.js/db-write.js будут ссылаться на удалённые ниже таблицы и
-- падать в рантайме (не на DEPLOY — plpgsql-тела не валидируются при
-- CREATE FUNCTION). Это ожидаемо при строгом порядке MIGR-1→2/3/4.
-- ════════════════════════════════════════════════════════════
