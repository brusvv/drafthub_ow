-- ════════════════════════════════════════════════════════════
-- DraftHub OW — 002_functions_and_rls.sql
-- Функции, RPC, RLS политики.
...
-- Получить sort_order конкретной роли — SECURITY DEFINER чтобы обойти
-- RLS на roles внутри WITH CHECK политики team_invites
...
-- ════ СИСТЕМНЫЕ РОЛИ — триггер при создании команды ════
...
-- ── get_team_members (SECURITY DEFINER — читает auth.users напрямую) ──
...
-- ── view_shared_tier (фаза 6: tier_set_name + фильтр по tier_set_id) ──
...
-- team_invites — используем role_sort_order() SECURITY DEFINER вместо inline subquery
