...
async function loadHeroes(){
  const rows=await sGet('Heroes!A:G');
  if(rows.length<2){heroes=[];return}
  const[h,...d]=rows;
  const i=f=>h.findIndex(x=>x.toLowerCase()===f.toLowerCase());
  const iN=i('name'),iR=i('role'),iS=i('subrole'),iP=i('priority'),iB=i('banned'),iNo=i('notes');
  heroes=d.filter(r=>r[iN]).map((r,idx)=>({rowIndex:idx+2,name:r[iN]||'',role:r[iR]||'',subrole:r[iS]||'',priority:parseInt(r[iP])||5,banned:(r[iB]||'').toUpperCase()==='TRUE',notes:r[iNo]||''}));
  heroMap={};heroes.forEach(h=>{heroMap[h.name]=h});
}

async function loadMaps(){
  const[mr,pr,br,cr]=await Promise.all([sGet('Maps!A:H'),sGet('MapPreferred!A:B'),sGet('MapBans!A:B'),sGet('Compositions!A:C')]);
  const pf={},bn={},co={};
...
async function seedSheets(){
  ...
  try{
    await sUp('Heroes!A1:F'+SH.length,SH);
