// @hash da342244 2026-07-16T10:11
// ════ TIER SHARE PUBLIC — обработка /tier/TOKEN, просмотр без авторизации ════
// Вынесено из render-tier-share.js (FILESPLIT, 16.07 — было 410 строк, см.
// render-tier-share-panel.js за подробностями границы). Личный кабинет
// владельца (создание/удаление share-ссылок) — соседний файл, не пересекается
// по состоянию: здесь своя пара module-level переменных (_shared*), страница
// рендерится в document.body.innerHTML целиком, а не в #app.
//
// Зависимости: db-load.js (loadSharedTier, _loadCatalogs),
//              render-portraits/maps (loadPortraits, loadMapScreenshots, loadWikiIcons),
//              basePathRegex (core/config.js)
