...
⚠️ **Текущий статус (26.06.2025):** `003_rls.sql` Пользователь добавил в архив правильный 003.
...
```
### Дубли CSS между файлами и порядок cascade

```bash
# По умолчанию читает CSS_FILES из build.sh, поэтому сравнивает правила
# ровно в порядке подключения в dist/index.html.
python3 scripts/css_dupe_check.py

# Необязательно: включить правила внутри @media для отдельной ручной проверки.
python3 scripts/css_dupe_check.py --include-media
```

Скрипт анализирует одинаковые простые селекторы (`.class`, `#id`,
`.class:pseudo`). Для одинакового селектора specificity одинаковая, поэтому
при равной важности побеждает правило из более позднего файла `build.sh`.
В отчёте указывается файл/строка победителя и перетёртых свойств. Правила в
`@media` по умолчанию исключены, поскольку они часто намеренно применяются
при разных условиях.
---
...
supabase/
  ...
  003_rls.sql          — ⚠️ Пользователь добавил в архив правильный 003
  004_rpc.sql          — 11 RPC функций, SECURITY DEFINER + search_path ✅
...
| 7 | `003_rls.sql` в архиве — неправильный файл (баг передачи bor) | Критично | ⚠️ Пользователь добавил в архив правильный 003 |
